...
from InteractiveSystem.models.base import get_model
...
def load_config():
    ...
    # Set up translations
    language = parsed_ini.get('InteractiveSystem', 'LANGUAGE')
    translate = gettext.translation('interactiveSystem',
                                    os.path.join(os.path.abspath(os.path.dirname(__file__)), "locales"),
                                    fallback=True,
                                    languages=[language])

    # Select model
    requested_model = parsed_ini.get('InteractiveSystem', 'MODEL')
