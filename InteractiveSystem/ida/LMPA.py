...
from InteractiveSystem.ida.Prompts import chosen_func_prompt, comment_prompt
from InteractiveSystem.ida.ida_helpers import get_format
...
from InteractiveSystem.ida.Tree import Tree
class LMPAHandler(idaapi.action_handler_t):
    ...
    def recover_function_name_args_iteratively(self, iterations: int):
        ...
        if Root.isLeaf:
            ...
            # interact with GPT
            response = InteractiveSystem.config.model.query_model_sync(
                _(chosen_func_prompt).format(decompiler_output=Root.body, params=params, format=(get_format(Root))))
            cnt = 5
            while not json.loads(response) and cnt > 0:
                response = InteractiveSystem.config.model.query_model_sync(
                    _(chosen_func_prompt).format(decompiler_output=Root.body, params=params, format=(get_format(Root))))
                cnt -=1
            ...
            while (iterations >= 0):
                ...
                for edge in self.Tree.G.edges(node_id):
                    ...
                    # interact with GPT
                    response = InteractiveSystem.config.model.query_model_sync(comment + prompt)
