...
def comment_callback(address, view, response):
    ...
    # Add the response as a comment in IDA, but preserve any existing non-InteractiveSystem comment
    comment = idc.get_func_cmt(address, 0)
    comment = re.sub(
        r'----- ' + _("Comment generated by System") + ' -----.*?----------------------------------------',
        r"",
        comment,
        flags=re.DOTALL)

    idc.set_func_cmt(address, '----- ' + _("Comment generated by System") +
                     f" -----\n\n"
                     f"{response.strip()}\n\n"
                     f"----------------------------------------\n\n"
                     f"{comment.strip()}", 0)
    ...
    print(_("{model} query finished!").format(model=str(InteractiveSystem.config.model)))
...
class ExplainHandler(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        InteractiveSystem.config.model.query_model_async(
...
def rename_callback(address, view, response, retries=0):
    ...
    print(_("{model} query finished! {replaced} variable(s) renamed.").format(model=str(InteractiveSystem.config.model),
...
class SwapModelHandler(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        InteractiveSystem.config.model = get_model(self.new_model)
        InteractiveSystem.config.update_config("InteractiveSystem", "MODEL", self.new_model)
